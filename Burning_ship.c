...
void ft_init_burning_ship(t_fractol *fractal)
{
	fractal->new_z_im = 0;
...
static int	calculate_z(t_fractol *fractal, int x, int y)
{
	...
	while (i < fractal->max_iter)
	{
		...
		fractal->new_z_im = 2 * fabs(fractal->old_z_real * fractal->old_z_im) + fractal->const_im;
		if ((fractal->new_z_real * fractal->new_z_real + fractal->new_z_im * fractal->new_z_im) > 10)
			break;
		i++;
	}
	return (i);
...
void ft_burning_ship(t_vars *vars_mlx)
{
	...
	y = 0;
	i = 0;
	ft_init_burning_ship(&vars_mlx->fractal);
